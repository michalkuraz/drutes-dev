module netcdfflux
  ...
  subroutine ncflux_init(ok)
    ...
    ierr = nf90_inq_varid(netcdfID, "lon", ncfluxdata%lon_varid)
    if (ierr /= nf90_noerr) return

    ierr = nf90_inq_varid(netcdfID, "lat", ncfluxdata%lat_varid)
    if (ierr /= nf90_noerr) return

    ierr = nf90_inq_varid(netcdfID, "time", ncfluxdata%time_varid)
    if (ierr /= nf90_noerr) return

    ierr = nf90_inq_varid(netcdfID, "Qrouted", ncfluxdata%q_varid)
    if (ierr /= nf90_noerr) return

    ierr = nf90_inq_dimid(netcdfID, "lon", dimid)
    if (ierr /= nf90_noerr) return
    ierr = nf90_inquire_dimension(netcdfID, dimid, len=n_default)
    if (ierr /= nf90_noerr) return
    ncfluxdata%nlon = int(n_default, kind=ikind)

    ierr = nf90_inq_dimid(netcdfID, "lat", dimid)
    if (ierr /= nf90_noerr) return
    ierr = nf90_inquire_dimension(netcdfID, dimid, len=n_default)
    if (ierr /= nf90_noerr) return
    ncfluxdata%nlat = int(n_default, kind=ikind)

    ierr = nf90_inq_dimid(netcdfID, "time", dimid)
    if (ierr /= nf90_noerr) return
    ierr = nf90_inquire_dimension(netcdfID, dimid, len=n_default)
    if (ierr /= nf90_noerr) return
  ...
  subroutine ncflux_get_xy(x, y, cur_hrs, qval, ok)
    ...
    if (.not. ok) then
      print *, "ncflux_get_xy: requested time not found, cur_hrs = ", cur_hrs
      return
    end if

    call load_qslice(tidx, ok)
    if (.not. ok) then
      print *, "ncflux_get_xy: failed to load Qrouted slice, time index = ", tidx
      return
    end if

    call utm2latlong(x, y, lat0, lon0)

    call interpolate_flux_latlon(lat0, lon0, qval, ok)
    if (.not. ok) then
      print *, "ncflux_get_xy: point outside flux.nc domain or fill value encountered"
      print *, "  x, y     = ", x, y
      print *, "  lat, lon = ", lat0, lon0
      print *, "  lat range = ", minval(ncfluxdata%lat), maxval(ncfluxdata%lat)
      print *, "  lon range = ", minval(ncfluxdata%lon), maxval(ncfluxdata%lon)
    end if
  end subroutine ncflux_get_xy


  subroutine load_qslice(tidx, ok)
    ...
    allocate(tmp(1, ncfluxdata%nlat, ncfluxdata%nlon))

    start = (/ int(tidx), 1, 1 /)
    count = (/ 1, int(ncfluxdata%nlat), int(ncfluxdata%nlon) /)
  ...
  subroutine interpolate_flux_latlon(lat0, lon0, qval, ok)
